...
update im_categories set category_description = 'trans_edit_wf' where category_id = 87;
update im_categories set category_description = 'edit_only_wf' where category_id = 88;
update im_categories set category_description = 'trans_edit_proof_wf' where category_id = 89;
update im_categories set category_description = 'localization_wf' where category_id = 91;
update im_categories set category_description = 'trans_only_wf' where category_id = 93;
update im_categories set category_description = 'trans_spotcheck_wf' where category_id = 94;
update im_categories set category_description = 'proof_only_wf' where category_id = 95;
update im_categories set category_description = 'glossary_compilation_wf' where category_id = 96;
...
declare
	...
	p_callback		alias for $3;
	p_custom_arg		alias for $4;
	p_notification_type	alias for $5;
	...
	v_count					integer;
begin
	RAISE NOTICE 'workflow_case__notify_assignee: task_id=%, user_id=%, callback=%, custom_arg=%, notification_type=%', p_task_id, p_user_id, p_callback, p_custom_arg, p_notification_type;

	-- Default notification type
	v_notification_type := p_notification_type;
	...
	v_workflow_url := v_workflow_url || 'task?task_id=' || p_task_id;
	...
	IF v_count > 0 THEN
		...
		RAISE NOTICE 'workflow_case__notify_assignee: interval=%: No notification type defined=%', v_notification_n_seconds, v_notification_type;
	...
	IF p_callback != '' AND p_callback is not null THEN
		v_str :=  'select ' || p_callback || ' (' ||
			p_task_id || ',' ||
			coalesce(quote_literal(p_custom_arg),'null') || ',' ||
			p_user_id || ',' ||
			v_party_from || ',' ||
